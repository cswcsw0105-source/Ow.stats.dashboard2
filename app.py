...
st.write('내가 직접 만든 오버워치 데이터 분석 대시보드 v5.0 (생태계 팩트 체크 추가)')
st.divider()
...
# 💻 8. 전 시즌 생태계(메타/카운터) 분석 및 맞춤형 피드백
st.divider()
st.subheader("💡 시즌별 생태계 분석 및 맞춤 피드백")

analyze_season = st.selectbox("상세 분석을 원하는 시즌을 선택하세요", current_seasons)
idx_analyze = current_seasons.index(analyze_season)

st.markdown(f"**[{analyze_season} 내 기록]** ⚔️ KDA: {current_kda[idx_analyze]} | 🎯 명중률: {current_acc_hip[idx_analyze]}% | 🏆 승리: {current_wins[idx_analyze]}승")
...
patch_and_meta_data = {
    "캐서디": {
        ...
        "19시즌": "🔻 **[카운터 버프/메타]** 윈스턴, 겐지를 필두로 한 하드 다이브 조합이 득세. 뚜벅이 딜러들의 지옥 같은 '억까' 메타 도래.\n\n🔥 **[분석]** 7시간이나 묵묵히 버티며 명중률은 16시즌과 동일한 53%를 방어했습니다! KDA가 2.55에 머문 것은 실력 탓이 아닙니다. 집중 포커싱을 당하는 지옥의 생태계 속에서도 에임 피지컬은 굳건했습니다.",
    ...
    "아나": {
        ...
        "18시즌": "➖ **[변화 없음]** 생태계 변동 없음.\n\n💡 **[분석]** KDA 1.33으로 회복세. 패치 변화가 없는 상태에서 지표가 올랐다는 것은 폼이 다시 올라오고 있었다는 증거입니다.",
        "19시즌": "🔥 **[핵심 패치]** 인간사냥꾼 특전 헤드샷 데미지 배율 1.5배 → 2배 대폭 상향!\n\n💡 **[분석]** 버프 타이밍을 정확히 캐치하여 5시간(25판) 14승을 쓸어 담았습니다. 억까 메타 속에서도 딜러 뺨치는 킬 포텐셜로 팀을 캐리 했습니다.",
